// security.js - BY ORDER - UTAK - 20 YEARS - LOCK
// Pera wala sa app - nasa Google Play / PayMongo direct - MPoints lang nasa app pang Boost Badge
...
const Security = {
  ...
  // free limits - by order
  FREE_MAX_POST: 3,
  BASIC_MAX_POST: 100,
  GOLD_MAX_POST: 500,

  // ref - tama - doc - hindi collection - tugma sa firestore.rules
  async getRef(uid){
  ...
  async addFail(uid, email){
    try{
      ...
      if(fails >= this.MAX_FAILS){
        lockUntil = Date.now() + this.LOCK_MINUTES * 60 * 1000;
      }
      await ref.set({
        fails: fails,
        lastFail: Date.now(),
  ...
  // client UX only - hindi security - pang loading lang
  isClientLocked(){
  ...
  // dom lock - by order - pag inalis sa DevTools - LOCKED
  checkDomLock(){
    ...
    if(m.dataset.modulock !== "DASHBOARD") return true;
    return false;
  },

  // sanitizer - bawal script - bawal illegal - by order safe
  sanitize(text){
    if(!text) return "";
    return text.toString().replace(/<[^>]*>/g,"").slice(0,500);
  },

  // check free 3 max - by order - client check - server final check sa rules + function
  async canPost(uid){
    try{
      if(!uid) return false;
      ...
      const role = d.role || "free";
      if(role === "gold") return total < 500;
      if(role === "basic") return total < 100;
      return total < 3;
    }catch(e){
      return true;
    }
